...
### Requirement: Task list scrolling
**Reason**: Task list no longer supports cursor navigation; it is display-only
**Migration**: No user action needed; feature removed

### Requirement: Output viewport with real-time streaming
**Reason**: Replaced by progress.md file display
**Migration**: Agent output is captured in progress.md; users see cumulative log instead of per-iteration streaming

### Requirement: Output buffer cleared per iteration
**Reason**: No longer applicable; progress.md accumulates content across iterations
**Migration**: No user action needed; behavior change is intentional
