...
The system SHALL provide an `upgrade` command that checks for the `openspec` prerequisite and applies init improvements including OpenSpec setup to existing projects.
...
#### Scenario: Upgrade fails if openspec not installed
- **WHEN** user runs `littlefactory upgrade` and `openspec` is not found in PATH
- **THEN** system prints error message and exits with non-zero code without modifying any files

#### Scenario: Successful upgrade
- **WHEN** user runs `littlefactory upgrade` in a directory with Factoryfile and `openspec` in PATH
- **THEN** system applies AGENTS.md setup, gitignore updates, skill installation, and OpenSpec setup
...
- **WHEN** upgrade runs
- **THEN** system prints numbered steps [1/4] through [4/4] with indented sub-operations for each change made or skipped
