...
## What It Does
The run command supports --tasks and --change flags to control which task source file is used. The --tasks flag provides an explicit path, --change points to a named change under the state directory, and the default falls back to the state directory tasks.json.
...
- **THEN** System uses `.littlefactory/changes/feature-a/tasks.json`
