...
## What It Does
Tasks are stored in a JSON file at a configured or explicitly specified path. The file contains a tasks array with structured objects supporting id, title, description, status, labels, and blockers fields. The source validates on load and supports both config-based and explicit path initialization.
