...
## What It Does
Executes agent subprocesses inside a pseudo-terminal (PTY) so that the subprocess believes it is running in a real terminal. This preserves color output and terminal features while still allowing littlefactory to capture and measure the output.
